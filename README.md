...
- プリプロセッサ（`#include`、`#define`、関数形式マクロ、`#undef`、`#ifdef`/`#ifndef`/`#if`/`#elif`/`#else`/`#endif`）。
- ビットフィールド宣言（現在の実装ではビット単位パッキングは未対応で、独立メンバとして扱います）。

## C99未実装（優先度4以降）

- `long double`、`_Complex`、可変長配列 (VLA)。
- 指定イニシャライザ（`[i]=` や `.member=`）と複合リテラル。
- `goto`/ラベル、`switch` の一部高度ケース、関数ポインタの複雑な宣言・呼び出し。
- プリプロセッサの高度機能（`#error`、`#pragma`、`##`、`#`、再帰展開の完全互換など）。
