...
- `inline` 関数（LLVM `alwaysinline` 属性を付与）。`static` 関数（LLVM `PrivateLinkage` を設定）。`restrict` 修飾子付きポインタパラメータ（LLVM `noalias` 属性を付与）。`volatile` 修飾子付き変数（LLVM volatile load/store を生成）。
...
1. **標準ライブラリヘッダ** `<complex.h>`, `<tgmath.h>`, `<fenv.h>`, `<wctype.h>`, `<wchar.h>` などが提供されていません（`selfhost/include/` に最小限のものが用意されているのみです）。
2. **可変長配列 (VLA) の高度な利用** 基本的なローカル1次元配列は可能ですが、多次元 VLA、VLA の初期化式、関数引数での `static` 修飾などは未実装です。
3. **複素数型 (`_Complex`, `_Imaginary`)** キーワードは認識しますが、現在は実数部のみをスカラー（`float`/`double`）として処理する不完全な実装です。`_Imaginary` 型は未サポートです。
4. **`long double` の精度** パースされますが、実質的には `double`（LLVM の `double`）と同じ精度で処理されています。
5. **複合リテラルの自動記憶域期間** C99 ではブロックスコープの複合リテラルは自動記憶域期間を持ちますが、現在の実装は常に新しいオブジェクトを生成します。
6. **ワイド文字・文字列リテラル** (`L'a'`, `L"abc"`) `wchar_t` および関連するリテラル表記はサポートされていません。
7. **ユニバーサル文字名** (`\uXXXX`, `\UXXXXXXXX`) ソースコード内での Unicode 文字コード指定は未対応です。
